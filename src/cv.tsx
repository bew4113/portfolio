...
const TERMINAL_SCRIPT: TerminalLine[] = [
  { text: '# secops terminal | pipeline.yml deployment session', color: 'text-slate-500' },
  { text: '$ pwd', color: 'text-cyan-300' },
  { text: '/opt/secops', color: 'text-violet-300' },
  { text: '$ ls -la', color: 'text-cyan-300' },
  { text: 'drwxr-xr-x soc-ai  docker-compose.soc.yml  scripts  logs', color: 'text-violet-300' },
  { text: '$ cd /opt/secops/soc-ai', color: 'text-cyan-300' },
  { text: '$ ls', color: 'text-cyan-300' },
  { text: 'pipeline.yml  vector.yaml  playbooks/  dashboards/', color: 'text-violet-300' },
  { text: '$ cat pipeline.yml', color: 'text-cyan-300' },
  { text: 'source: crowdstrike-logscale | sink: clickhouse,minio', color: 'text-violet-300' },
  { text: '$ grep -n "playbook\|graph" pipeline.yml', color: 'text-cyan-300' },
  { text: '18: playbook: ransomware_auto_isolate | 27: graph_sync: true', color: 'text-violet-300' },
  ...
  { text: '$ systemctl status vector --no-pager | head -n 5', color: 'text-cyan-300' },
  { text: 'active (running) since Tue 2026-04-28 08:11:02 UTC', color: 'text-emerald-400' },
  { text: '$ docker compose -f /opt/secops/docker-compose.soc.yml ps', color: 'text-cyan-300' },
  { text: 'vector  Up | clickhouse  Up | minio  Up | n8n  Up', color: 'text-emerald-400' },
  { text: '$ docker compose -f /opt/secops/docker-compose.soc.yml logs --tail=15 vector', color: 'text-cyan-300' },
  { text: '[info] processed=2.4m/s dropped=0 retries=0', color: 'text-fuchsia-300' },
  { text: '$ clickhouse-client -q "SELECT count() FROM siem.logs WHERE ts >= now()-INTERVAL 5 MINUTE"', color: 'text-cyan-300' },
  { text: '7219843', color: 'text-fuchsia-300' },
  ...
  { text: '$ mc ls soc/log-archive/secops/', color: 'text-cyan-300' },
  { text: '[2026-04-28 09:10:31]  14GiB STANDARD 2026-04-28/', color: 'text-violet-300' },
  { text: '$ /var/ossec/bin/wazuh-control status', color: 'text-cyan-300' },
  { text: 'wazuh-manager: running | wazuh-analysisd: running', color: 'text-emerald-400' },
  { text: '$ journalctl -u wazuh-manager -n 10 --no-pager', color: 'text-cyan-300' },
  { text: 'rule_id=5710 level=7 srcip=10.10.22.14 action=correlated', color: 'text-violet-300' },
  { text: '$ sudo /opt/CrowdStrike/falconctl -g --aid', color: 'text-cyan-300' },
  { text: 'AID="2f8d9a5a2f1d4e5f9abce123456789ab"', color: 'text-emerald-400' },
  { text: '$ nmap -sV -Pn -p 22,80,443 10.10.20.15', color: 'text-cyan-300' },
  { text: '22/tcp open ssh | 80/tcp open http | 443/tcp open https', color: 'text-violet-300' },
  { text: '$ nc -vz 10.10.20.15 443', color: 'text-cyan-300' },
  { text: 'Connection to 10.10.20.15 443 port [tcp/https] succeeded!', color: 'text-emerald-400' },
  { text: '$ wafw00f https://portal.secops.local', color: 'text-cyan-300' },
  { text: '[+] The site is behind Cloudflare (Cloudflare Inc.)', color: 'text-emerald-400' },
  ...
  { text: '$ pwsh -Command "Get-MgUser -Top 3 | Select DisplayName,UserPrincipalName"', color: 'text-cyan-300' },
  { text: 'Panupong Nijjaboon  panupong.nijjaboon@tenant.onmicrosoft.com', color: 'text-violet-300' },
  { text: '$ n8n start', color: 'text-cyan-300' },
  { text: 'Editor is now accessible via: http://localhost:5678', color: 'text-emerald-400' },
  { text: '$ git -C /opt/secops/soc-ai status --short', color: 'text-cyan-300' },
  { text: 'M pipeline.yml | M playbooks/ransomware_auto_isolate.ts', color: 'text-violet-300' },
  { text: '$ npm run build', color: 'text-cyan-300' },
  { text: 'vite build complete in 1.8s | zero type errors', color: 'text-emerald-400' },
  { text: '$ echo "SOC automation pipeline online"', color: 'text-cyan-300' },
  { text: 'SOC automation pipeline online', color: 'text-emerald-400' },
];
